...
def generate_frame():
    ...
    f_class = dt.time(10, 0, 0)
    s_class = dt.time(23, 41, 0)
    t_class = dt.time(10, 42, 0)
    l_class = dt.time(13, 0, 0)
                ...
                if counter != 0:
                    if counter == 1:
                       ...
                       # if secondElapsed > 1:
                        ...
                        if (f_class < curr_time < s_class):
                            ref = db.reference(f"Students/{id}")
                            if studentInfo['Web_Attendance']!= datetime.now().strftime("%Y-%m-%d"):
                                studentInfo['Web_Tech'] += 1
                                ref.child('Web_Tech').set(studentInfo['Web_Tech'])
                                ref.child("Web_Attendance").set(datetime.now().strftime("%Y-%m-%d"))
                            else:
                                modeType = 3
                                counter = 0
                                imgBackground[44 : 44 + 633, 808 : 808 + 414] = imgModeList[modeType]
                        ...
                        elif (s_class < curr_time < t_class):
                            ref = db.reference(f"Students/{id}")
                            if studentInfo['Software_Attendance']!= datetime.now().strftime("%Y-%m-%d"):
                                studentInfo['SoftWare_Testing'] += 1
                                ref.child('SoftWare_Testing').set(studentInfo['SoftWare_Testing'])
                                ref.child("Software_Attendance").set(datetime.now().strftime("%Y-%m-%d"))
                            else:
                                modeType = 3
                                counter = 0
                                imgBackground[44 : 44 + 633, 808 : 808 + 414] = imgModeList[modeType]
                        ...
                        elif (t_class < curr_time < l_class):
                            ref = db.reference(f"Students/{id}")
                            if studentInfo['Acc_Attendance']!= datetime.now().strftime("%Y-%m-%d"):
                                studentInfo['Accounting Tools'] += 1
                                ref.child('Accounting Tools').set(studentInfo['Accounting Tools'])
                                ref.child("Acc_Attendance").set(datetime.now().strftime("%Y-%m-%d"))
                            else:
                                modeType = 3
                                counter = 0
                                imgBackground[44 : 44 + 633, 808 : 808 + 414] = imgModeList[modeType]
                            ...
                            already_marked_id_student.append(id)
                            already_marked_id_admin.append(id)
                            cv2.putText(imgBackground,str("No Classes Now"),(861, 125),cv2.FONT_HERSHEY_COMPLEX,1,(0, 0, 0),1,)
                                # marked()
                    ...
                    if modeType != 3:
                        ...
                        if counter <= 5:
                            ...
                            cv2.putText(imgBackground,str(studentInfo["Degree"]),(1006, 550),cv2.FONT_HERSHEY_COMPLEX,0.5,(255, 255, 255),1,)
                            cv2.putText(imgBackground,str(id),(1006, 493),cv2.FONT_HERSHEY_COMPLEX,0.5,(255, 255, 255),1,)
                            cv2.putText(imgBackground,str(studentInfo["CGPA"]),(910, 625),cv2.FONT_HERSHEY_COMPLEX,0.6,(100, 100, 100),1,)
                            cv2.putText(imgBackground,str(studentInfo["Curr_year"]),(1025, 625),cv2.FONT_HERSHEY_COMPLEX,0.6,(100, 100, 100),1,)
